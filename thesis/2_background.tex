...
\begin{document}
    ...
    \[ \includegraphics[scale=0.5]{images/two_view.jpg} \]

    \[ x'_{c} &= f\frac{X}{Z} \quad,\quad x_{c} &= f\frac{X+B}{Z} \]
    \[ d &= x_{c} - x'_{c} &= \frac{fB}{Z} \]
    \[ Z &= \frac{fB}{d} \]

    Let disparity be the difference in the coordinates of a pixel in the two images. Here, the only challenge is to
    ...
    cross corresponded keypoints in each image plane. Recalling the epipolar geometry picture above, the epipolar
    ...
    position of the 3D point can be caluclulated. This process is also known as triangulation.
